...
struct StatsView: View {
    ...
    // Equatable animation key for monthTop3Books
    private struct TopBookAnimKey: Equatable {
        let book: String
        let seconds: Int
    }
    private var monthTop3AnimationKey: [TopBookAnimKey] {
        monthTop3Books.map { TopBookAnimKey(book: $0.book, seconds: $0.seconds) }
    }
                ...
                if monthTop3Books.isEmpty {
                    ...
                    VStack(spacing: 10) {
                        ForEach(Array(monthTop3Books.enumerated()), id: \.offset) { idx, entry in
                            ...
                            .accessibilityLabel("\(idx + 1). \(entry.book), \(BibleStatsStore.shared.format(entry.seconds)) this month")
                            .transition(.opacity.combined(with: .move(edge: .top)))
                        }
                    }
                    .animation(.spring(response: 0.35, dampingFraction: 0.85), value: monthTop3AnimationKey)
                }
            }
            .padding(.top, 2)
        }
        ...
        VStack(alignment: .leading, spacing: rowSpacing) {
            HStack(spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(BibleStatsStore.shared.format(seconds))
